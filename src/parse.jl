...
"""
    build_tree!(template::String, current_index::Int, node::Node, 
                stack::Vector{Julja2.TokenType}, options::TemplateOptions)
...
## Example:
```julia
template = " root_string {% for <body> %} for_string{% if true %} " *
...
- `options::TemplateOptions`: Options for template parsing and rendering.
"""
function fill_node_template_string!(
    node::Node, token::String, token_type::TokenType, options::TemplateOptions
)
    if token_type == string_token
        node.template_string = "\nwrite(_result, \"" * token * "\")"
    elseif token_type == variable_token
        if options.ignore_missing_vars
...
## Returns:
- `String`: The generated code from the AST nodes.
"""
function build_code(node::Node)::String
    if isempty(node.children)
        return node.template_string
    else
        child_codes = map(build_code, node.children)
